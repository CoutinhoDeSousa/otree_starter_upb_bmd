...
class C(BaseConstants):
    ...
    NUM_ROUNDS = 3
    NAME_IN_URL = 'guess_two_thirds_vhb'
    JACKPOT = Currency(100)
    GUESS_MAX = 100
    INSTRUCTIONS_TEMPLATE = 'guess_two_thirds_vhb/instructions.html'
...
class FinalResults(Page):
    ...
    def vars_for_template(player: Player):
        return {
            'mytotalpayoff': sum([p.payoff for p in player.in_all_rounds()])
        }

page_sequence = [Introduction, Guess, ResultsWaitPage, Results, FinalResults]
